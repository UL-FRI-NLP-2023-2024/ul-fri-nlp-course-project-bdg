...
path_to_data = os.path.join('../../datasets/dataset_alter/')
path_to_save_corpus = os.path.join('../../corpora/corpus_alter/')
path_to_save_our_model = os.path.join('../../corpora_model_fine-tune/corpus_model_fine-tune_alter')
...
for filename in os.listdir(path_to_data):
    ...
    # add ratings to messages, based on author_role
    # add column role_rating
    data['role_rating'] = 0
    for index, row in data.iterrows():
        if row['author_level'] in role_ratings:
            data.at[index, 'role_rating'] = int(role_ratings[row['author_level']])
        else:
            data.at[index, 'role_rating'] = int(1)
    ...
    line_separators_pattern = r'[\u2028\u2029]'

    for index, row in data.iterrows():

        msg = row['message']
        msg = msg.strip()
        # remove unicode empty spaces
        msg = re.sub(r'\s+', ' ', msg).strip()
        msg = re.sub(line_separators_pattern, ' ', msg).strip()
        ...
        # special case - citation
            ...
            while(msg != ""):
                ...
                # save the citation
                list_corpus.append({'index': corpus_index, 'source': source, "role": "user", 'prompt': citation_prompt.strip(), 'answers': [{'role': "assistant", 'message': citation_answer.strip(), "answer_rating": int(row["role_rating"]), "answer_hate": 0}]})
                corpus_index += 1
        ...
        # special case - mention assistant
            ...
            if (exact_assistant == row['user']):
                ...
                if (row['user'] == prompt_user):
                    ...
                    answers.append({'role': "assistant", 'message': row['message'].strip(), "answer_rating": int(row['role_rating']), "answer_hate": 0})
            else:
                mention_prompt = ""
        ...
        # new prompt
            ...
            if (is_question):
                ...
                # answer
                new_answer = [{'role': "assistant", 'message': row['message'].strip(), "answer_rating": int(row["role_rating"]), "answer_hate": 0}]
                # prompt is previous
    ...
    if (len(answers) == 0):
        answers.append({'role': "assistant", 'message': prompt.strip(), "answer_rating": int(data.iloc[-1]["role_rating"]), "answer_hate": 0})
...
# save the lists
save_path = os.path.join(path_to_save_corpus, "corpus_alter.json")
with open(save_path, 'w', encoding='utf-8') as f:
    json.dump(list_corpus, f, indent=4, ensure_ascii=False)

save_path = os.path.join(path_to_save_our_model, "model_finetune_alter.json")
with open(save_path, 'w', encoding='utf-8') as f:
